#!/usr/bin/env bash

cd test-server
npm install
cd ..

$(npm bin)/screeps start \
  --db test-server/db.json \
  --logdir test-server/logs \
  --modfile test-server/mods.json \
  --assetdir test-server/assets \
  --cli_host localhost \
  --host 127.0.0.1 \
  --password tooangel \
  &

sleep 5
echo 'Spawn bot'
echo "bots.spawn('screeps-bot-tooangel', 'W1N7', {username: 'TooAngel', cpu: 100, gcl: 1, x: 43, y: 35})" | $(npm bin)/screeps cli &
sleep 2
echo "storage.db.users.update({username: 'TooAngel'}, {\$set: {password: '', salt: '7eeb813417828682419582da8f997dea3e848ce8293e68b2dbb2f334b1f8949f'}})" | $(npm bin)/screeps cli &
sleep 3
node utils/check_private_server.js &
echo "system.resumeSimulation()" | $(npm bin)/screeps cli &
echo $PID
sleep 300
echo "storage.db['rooms.objects'].find({room: 'W1N7', type: 'controller'})" | $(npm bin)/screeps cli > output.log &
sleep 5
kill %1 %2 %3 %4

grep "progress:" output.log
if grep -q "progress: 0" output.log;
then
    echo '!!! No progress on the controller !!!'
    rm output.log
    exit 1
else
    echo 'Yeah'
    rm output.log
    exit 0
fi
