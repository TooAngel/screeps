  ...
  socket.on('data', async (data) => {
    ...
    if (line.startsWith(`'OK'`)) {
      ...
      if (checkForStatus(status)) {
        ...
        console.log('checkForStatus 1 passed and failed');
      }
      await sleep(duration);
      if (checkForStatus(status)) {
        defer.resolve();
      } else {
        console.log('checkForStatus 2 passed and failed');
      }
      await sleep(duration);
      if (checkForStatus(status)) {
        defer.resolve();
      } else {
        console.log('checkForStatus 3 passed and failed');
      }
      await sleep(duration);
      if (checkForStatus(status)) {
        defer.resolve();
      } else {
        console.log('checkForStatus 4 passed and failed');
      }
      await sleep(duration);
      if (checkForStatus(status)) {
        defer.resolve();
      } else {
        console.log('checkForStatus 5 passed and failed');
      }
