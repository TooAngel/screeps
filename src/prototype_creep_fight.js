...
Creep.prototype.fleeFromHostile = function(hostile) {
  let direction = global.utils.oppositeDirection(this.pos.getDirectionTo(hostile));
  if (!direction || direction === null || this.pos.x === 0 || this.pos.x === 49 || this.pos.y === 0 || this.pos.y === 49) {
    this.moveTo(25, 25);
    return true;
  }
  for (let offset = 0; offset < 8; offset++) {
    const dir = global.utils.changeDirection(direction, offset);
...
Creep.prototype.flee = function(target) {
  let direction = global.utils.oppositeDirection(this.pos.getDirectionTo(target));
  this.rangedAttack(target);
