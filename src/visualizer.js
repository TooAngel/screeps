...
global.visualizer = {
    ...
    _.each(Game.rooms, (room, name) => {
      ...
      if (config.visualizer.showRoomPaths) {
        ...
        if (paths.length !== 0 && config.visualizer.showRoomPaths) {
          _.each(paths, route=> {
            visual.drawLine(route.path.map(p=>([p.x, p.y])), COLOR_WHITE, { lineWidth: 0.1 });
          });
        }
      }
      ...
      if (config.visualizer.showCreepPaths) {
        _.each(Game.creeps, creep=> {
          ...
          if (mem._move) {
            ...
            if (path.length) {
              visual.drawLine(path.map(p=>([p.x, p.y])), COLOR_RED, { lineWidth: 0.1 });
            }
          }
        });
      }
  ...
  showPath: function (path) {
    ...
    if (path.length) {
      visual.drawLine(path.path.map(p=>([p.x, p.y])), COLOR_RED, { lineWidth: 0.1 });
      visual.commit();
      return true;
    }
    return false;
