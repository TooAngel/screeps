...
Creep.prototype.getEnergyFromHostileStructures = function() {
  ...
  let hostileStructures = this.room.findOnlyStructType(FIND_HOSTILE_STRUCTURES, [STRUCTURE_CONTROLLER, STRUCTURE_RAMPART, STRUCTURE_EXTRACTOR]);
...
Creep.prototype.repairStructure = function() {
  ...
  if (nukes.length > 0) {
    let spawns = this.room.findOnlyStructType(FIND_MY_STRUCTURES, [STRUCTURE_SPAWN]);
    if (spawns.length > 0) {
